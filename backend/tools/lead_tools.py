...
async def upsert_from_chat(db: AsyncSession, session_id: str, state: dict) -> LeadResponse:
    """
    Create or update a Lead row from completed chat state.

    BUG FIX: address was read from state.get("location") instead of
    state.get("address"). All vertical states store the field as "address"
    (confirmed in every state.py). This silently wrote None for every lead.
    """
    ...
    if state.get("appt_confirmed"):
        try:
            appt_at = datetime.strptime(
                state["appt_confirmed"].split(", ", 1)[-1],
                "%b %d at %I:%M %p",
    ...
    if row is None:
        row = Lead(session_id=session_id, vertical=state.get("vertical", "hvac"))
        db.add(row)
    ...
    row.address = state.get("address") or row.address   # FIX: was "location"
    row.appt_at = appt_at or row.appt_at

    await db.commit()
    await db.refresh(row)
    logger.info("lead_upserted", session_id=session_id, vertical=row.vertical)
    return LeadResponse.model_validate(row)
