...
- phone: digits only, strip formatting. email: lowercase.
...
class AITools:
    ...
    async def _invoke_extract(self, system: str, message: str, service: str) -> dict | None:
        ...
        the system prompt. Extracted to a shared private method to eliminate
        ~60 lines of duplication. Each public method is now a one-liner.
        """
        ...
        try:
            ...
            if data:
                log.debug(f"{service}_ok", fields={k: v for k, v in data.items() if v is not None})
                return data
    ...
    async def extract_fields(self, last_user_message: str) -> dict | None:
        return await self._invoke_extract(_EXTRACT_SYSTEM, last_user_message, "extract_fields")
    ...
    async def extract_pest_fields(self, last_user_message: str) -> dict | None:
        ...
        return await self._invoke_extract(PEST_EXTRACT_SYSTEM, last_user_message, "extract_pest_fields")
    ...
    async def extract_plumbing_fields(self, last_user_message: str) -> dict | None:
        ...
        return await self._invoke_extract(PLUMBING_EXTRACT_SYSTEM, last_user_message, "extract_plumbing_fields")
    ...
    async def extract_roofing_fields(self, last_user_message: str) -> dict | None:
        ...
        return await self._invoke_extract(ROOFING_EXTRACT_SYSTEM, last_user_message, "extract_roofing_fields")
    ...
    async def score_lead(self, enrichment: LeadEnrichment) -> LeadScore:
        ...
        if enrichment.is_spam:
            return LeadScore(score="cold", score_reason="Spam or bot.", next_step="drop")
        if enrichment.urgency in ("emergency", "high"):
            return LeadScore(score="hot", score_reason="Active emergency.",
        ...
        try:
            ...
            data = parse_json(resp.content)
            if data:
                log.info("score_ok", score=data.get("score"), next_step=data.get("next_step"))
                return LeadScore(
        ...
        return LeadScore(score="warm", score_reason="Defaulted to warm.",
                         next_step="schedule_callback")

    async def enrich_lead(self, messages: list[dict]) -> Optional[LeadEnrichment]:
        """Legacy single-pass enrichment. Prefer vertical-specific extract_*_fields."""
        log = logger.bind(service="enrich_lead_legacy")
        last_user = next(
            (m["content"] for m in reversed(messages) if m.get("role") == "user"), None)
        if not last_user:
            return None
        fields, classification = await asyncio.gather(
            self.extract_fields(last_user),
            self.classify_conversation(messages),
        )
